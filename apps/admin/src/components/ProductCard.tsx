...
export default function ProductCard({ product, onAddToCart }: ProductCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Product Image */}
      <div className="relative w-full h-32 mb-3 bg-slate-50/50 rounded-xl overflow-hidden flex items-center justify-center">
        <Image
      ...
      <div className="space-y-1">
        ...
        {/* Product Name */}
        <h3 className="text-sm font-bold text-slate-800 line-clamp-2 leading-snug group-hover:text-primary transition-colors duration-150">
          {product.name}
        </h3>
          ...
          <AnimatePresence mode="wait">
            {quantity === 0 ? (
              <motion.button
                ...
                onClick={handleAdd}
                className="w-full bg-white border border-primary text-primary font-bold text-sm px-3 py-2 rounded-button hover:bg-primary hover:text-white shadow-sm hover:shadow transition-all duration-150"
              >
                ADD
              </motion.button>
            ) : (
