...
create table public.profiles (
    ...
    email text,
    avatar_url text,
    role text not null default 'customer' check (role in ('customer', 'staff', 'delivery', 'admin')),
...
-- 7b. Store Staff Access Codes (Generated by Admin for Store Staff Login)
create table public.store_staff_codes (
    id uuid default gen_random_uuid() primary key,
    store_name text default 'Main Store' not null,
...
create policy "Staff can view and update orders in store pipeline" 
  on public.orders for all using (
    exists (
      select 1 from public.profiles where profiles.id = auth.uid() and profiles.role in ('staff', 'admin')
    )
  );

create policy "Delivery partners can view and update active deliveries" 
  on public.orders for all using (
    exists (
      select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'delivery'
    ) and (status in ('ready_for_pickup', 'out_for_delivery', 'arrived', 'delivered') or delivery_partner_id = auth.uid())
  );

-- STORE STAFF CODES
create policy "Admins can manage store staff codes"
  on public.store_staff_codes for all using (
    exists (
      select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'admin'
    )
  );
